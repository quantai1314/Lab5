...
void calculateKernelResponse(int i, int j, int n, float *src, float *kernel, float *dst) {
    ...
    for (kj = 0; kj < KERNEL_SIZE; ++kj) { //! 列优先访问，性能较差
        for (ki = 0; ki < KERNEL_SIZE; ++ki) {
            // 映射到 src 的偏移位置
            int src_row = i + ki - KERNEL_SIZE / 2;
            int src_col = j + kj - KERNEL_SIZE / 2;
            // 边界检查：如果 src_row,src_col 超出 src 范围，则视为 0
            if (src_row >= 0 && src_row < n && src_col >= 0 && src_col < n) {//! 引入大量分支，影响性能
                pixel_value = src[src_row * n + src_col];
            }
            else {
                pixel_value = 0.0f; 
            }
...
{
    int i, j;
    int n_padded = n + KERNEL_SIZE - 1;
    float *padded_src = (float *)calloc(n_padded * n_padded, sizeof(float));
    ...
    for (i = 0; i < n; i++) {
        for (j = 0; j < n; ++j) {
            padded_src[(i + KERNEL_RADIUS) * n_padded + (j + KERNEL_RADIUS)] = src[i * n + j];
        }
    }

    for (i = 0; i < n; i++) {
        for (j = 0; j < n; j++) {
            accumulateKernelOnPadded(i, j, n_padded, padded_src, kernel, dst);
        }
    }  
...
void accumulateKernelOnPadded(int i, int j, int n_padded, float *padded_src, float *kernel, float *dst) {
    int n = n_padded - KERNEL_SIZE + 1;
    dst[i * n + j] = 0.0f;
    int ki, kj;
    float pixel_value;
    float sum = 0.0f;
    for (ki = 0; ki < KERNEL_SIZE; ki++) {
        for (kj = 0; kj < KERNEL_SIZE; kj++) {
            ...
            pixel_value = padded_src[src_row * n_padded + src_col];
